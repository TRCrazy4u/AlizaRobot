...
from AlizaRobot import DRAGONS, dispatcher
from AlizaRobot.modules.disable import DisableAbleCommandHandler
from AlizaRobot.modules.helper_funcs.admin_rights import user_can_changeinfo
from AlizaRobot.modules.helper_funcs.alternate import send_message
...
def promote(update: Update, context: CallbackContext) -> str:
    ...
    try:
        ...
        if err.message == "User_not_mutual_contact":
            ...
            message.reply_text(
                "An error occured while promoting."
            )
        return
...
def lowpromote(update: Update, context: CallbackContext) -> str:
    ...
    bot.sendMessage(
        chat.id,
        f"<b>Fake Promoting a user in </b>{chat.title}\n\n<b>New Sasta Admin:</b> {mention_html(user_member.user.id, user_member.user.first_name)}\nAdmin: {mention_html(user.id, user.first_name)}",
        parse_mode=ParseMode.HTML,
    )
...
def demote(update: Update, context: CallbackContext) -> str:
    ...
    if not user_member.status == "administrator":
        message.reply_text("Can't demote what wasn't promoted1")
        return
...
You Just Need To Give Commands To Bot And But Will Work for You. Click On Below Buttons & Get Detailed Information.
 ...
 *Admins only:*
 ...
 ❍ /lowpromote*:* promotes fake admin
 ...
 ❍ /setsticker*:* set the sticker pack for group
"""
