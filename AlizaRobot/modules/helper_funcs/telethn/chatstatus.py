...
from AlizaRobot import DRAGONS
from AlizaRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def aliza_is_admin(chat_id: int):
    status = False
    aliza = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if aliza.id == user.id:
