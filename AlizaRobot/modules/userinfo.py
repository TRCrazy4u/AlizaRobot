...
import AlizaRobot.modules.sql.userinfo_sql as sql
...
from AlizaRobot.modules.helper_funcs.extraction import extract_user
from AlizaRobot.modules.sql.afk_sql import check_afk_status, is_afk
from AlizaRobot.modules.sql.global_bans_sql import is_user_gbanned
from AlizaRobot.modules.sql.users_sql import get_user_num_chats
...
def get_id(update: Update, context: CallbackContext):
    ...
    if user_id:
        ...
        if chat.type == "private":
            msg.reply_text(
                f"Your User id is <code>{chat.id}</code>.", parse_mode=ParseMode.HTML
            )
            ...
            message.reply_document(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "Health", url="https://t.me/Aliza_basicinfo/5"
                            ),
                            InlineKeyboardButton(
                                "Disaster", url="https://t.me/Aliza_basicinfo/3"
                            ),
                        ],
                    ]
                ),
